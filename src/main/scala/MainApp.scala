...
object MainApp {
  ...
  def main(args: Array[String]) {
    ...
    if (args.length < 2) {
      ...
      //将zip文件转换成parquet文件
      val oneTest = List("2017-10-26","2017-11-03")
      oneTest.foreach(day=>singleFunction.AssemblyOne(day))
//      //输出每天的日志条数
//      singleFunction.AssemblyTwo()
//      //每天的搜索情况
//      val threeTest = List("2017-10-26","2017-11-03","2017-10-25","2017-11-02")
//      threeTest.foreach(day => singleFunction.AssemblyThree(day))
...
def baiduSearchAnalysis(df:DataFrame,day:String): Unit ={
  ...
  searchDF.write.mode("Overwrite").format("parquet").save(dataPath+"search.parquet/"+day)
  ...
  searchDF.select("targetUrl").filter(!$"targetUrl".contains("d=paperuri")).rdd
    ...
    .toDF()
    //.coalesce(1)
    .write.mode("Overwrite").format("text").save(dataPath+"/word.json/"+day)
  ...
  searchDF.select("targetUrl").filter($"targetUrl".contains("d=paperuri")).rdd
    ...
    .toDF()
    //.coalesce(1)
    .write.mode("Overwrite").format("text").save(dataPath+"/paper.json/"+day)
